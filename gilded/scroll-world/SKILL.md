...
description: >
  ...
  industry, brand, or Gilded Edge venture. As the visitor scrolls, a pre-rendered
  camera flies from outside each scene into its interior, then flows on to the next
  scene with NO cuts — one continuous connected flight (Emons-style isometric diorama
  world, photoreal luxury, or any art direction you pick). The skill interviews the
  user for the topic, the story beats/sections, and brand kit, then generates cohesive
  scenes + seamless camera clips with any supported provider — Higgsfield CLI, fal.ai
  (FAL_KEY, Seedance — the ecosystem default), Replicate, or Gemini/Veo; Higgsfield is
  NOT required — and wires a portable, framework-agnostic scroll engine in one of two
  render modes: video currentTime scrubbing or an Apple-style canvas frame-sequence
  (frame-atlas) scrubber. Use when the user wants a "3D world" /
  "browse-through-the-industry" hero, a scroll cinematic ("scroll cinematic for
  <venture>"), to "make our site scroll like Apple's", a scrollytelling / scroll-driven
  video hero, a diorama landing, or to turn a business into a scrollable world.
...
1. **Pick a generation provider** (Higgsfield is supported, not required). Detect in
   order and use the first hit:
   - `higgsfield` CLI on `$PATH` → upstream flow. If `higgsfield workspace list` fails
     auth, ask the user to run `higgsfield auth login` (interactive OAuth — you cannot
     run it) and, if needed, `higgsfield workspace set <id>`. Confirm credits.
   - `FAL_KEY` set → **fal.ai, the Gilded ecosystem default** (Seedance 2.0 i2v with
     start+end-image frame-locking — full skill, both architectures).
   - `REPLICATE_API_TOKEN` / `GEMINI_API_KEY` → Replicate / Veo, with honest
     frame-lock caveats (Veo is architecture-A-only unless verified).
   `references/providers.md` has the full matrix, runnable equivalents of every
   pipeline call, key checks that never print values, and the **cost preflight**
   (estimate the dollar spend and get explicit go-ahead / `--run` before any paid
   call — same convention as the cinematic-render-pipeline skill). Whatever the
   provider, a run is `N` image gens + `N` video gens (architecture A) or `(2N-1)`
   video gens (architecture B), plus re-rolls — the Step 1 budget tier sets `N`.
2. **ffmpeg / ffprobe** (frame extraction + encoding) — system `ffmpeg` if present,
   else the `ffmpeg-static` npm binary resolved from a venture's node_modules
   (media-pipeline skill conventions; discovery snippet in
   `references/frame-atlas.md`). Note `ffmpeg-static` ships no ffprobe — parse
...
**Two engines — pick per page.** `scrub-engine.js` (video `currentTime` scrubbing,
below) is the default: lightest assets, one file per clip.
`references/frame-scrub-engine.js` is a drop-in alternative that scrubs a
pre-extracted WebP frame sequence on a `<canvas>` — the technique Apple actually
ships: deterministic paint, no decoder seeks, immune to iOS Low Power Mode, perfect
reverse scrub — at the honest cost of heavier assets. Same config shape, pacing
knobs, seam doctrine, and `data-sw-seo` contract; choose it for hero pages or when
Step 8 QA shows scrub stutter. Extraction recipe + tradeoff table:
`references/frame-atlas.md`. (For Next.js / single-file / storyboard delivery, see
`references/export-targets.md`.) The rest of this step describes the video engine.
...
Gilded additions (see `GILDED-CHANGES.md` for the full changelog vs upstream):

- `references/providers.md` — provider matrix (Higgsfield / fal.ai / Replicate /
  Gemini+Veo) with runnable equivalents of every generation step, frame-lock
  capability notes, key checks that never print values, and the cost preflight.
- `references/frame-scrub-engine.js` — the frame-atlas render mode: canvas
  frame-sequence scrubber (Apple's shipped technique), same config shape as the
  video engine.
- `references/frame-atlas.md` — video-vs-frames tradeoff, ffmpeg frame-extraction +
  WebP recipe (bash 3.2 safe, ffmpeg-static discovery), QA deltas.
- `references/gilded-presets.md` — the Gilded house style preamble/palette + four
  venture world recipes (lumier-pictures, gilded-travel-works, Toutsweet,
  gilded-art-works) with lean/showcase journeys and copy starters.
- `references/export-targets.md` — Next.js App Router wrapper pattern, single-file
  HTML builds, and storyboard exports to the pptx/pdf skills.
